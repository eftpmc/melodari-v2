            ...
            {friendRequests.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {friendRequests.map((request: FriendRequest) => {
                        ...
                        if (!profile) {
                            return (
                                <div key={request.id} className="flex flex-col items-center justify-center h-32 bg-base-100 rounded-md shadow-md p-2">
                                    <p className="text-lg font-semibold text-base-content text-center">No Profile Available</p>
                                    <p className="text-sm text-gray-500 text-center">This friend request has no profile data.</p>
                                </div>
                            );
                        }
                            ...
                            <div key={request.id} className="flex flex-col sm:flex-row items-center p-4 bg-base-100 rounded-md shadow-md">
                                <div className="flex items-center">
                                    ...
                                    <div className="flex flex-col">
                                        <div className="text-base-content font-semibold">{profile.username || 'Unknown User'}</div>
                                        <div className="flex mt-2 space-x-2">
                                            {profile.platforms.map((platformId: string) => {
                    ...
                    {outgoingFriendRequests.map((request: FriendRequest) => {
                        ...
                        if (!profile) {
                            return (
                                <div key={request.id} className="flex flex-col items-center justify-center h-32 bg-base-100 rounded-md shadow-md p-2">
                                    <p className="text-lg font-semibold text-base-content text-center">No Profile Available</p>
                                    <p className="text-sm text-gray-500 text-center">This outgoing request has no profile data.</p>
                                </div>
                            );
                        }
